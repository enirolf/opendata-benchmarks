...
size_t getNEvents() {
  auto ntuple = RNTupleReader::Open("CollectionTree", "data/DAOD_PHYSLITE.rntuple.root");
...
void plot(int task, std::string_view taskKind) {
  ...
  for (const auto &format : {"ttree", "rntuple"}) {
    std::string resultsFilePath = "results/physlite/" + std::to_string(task) + "_" + format + "_" + std::string(taskKind) + ".data";
  ...
  title.DrawLatexNDC(0.5, 0.975, Form("PHYSLITE ADL benchmark, task %d (%s)", task, std::string(taskKind).c_str()));

  canvas.Print(Form("results/physlite/%d_throughput_%s.pdf", task, std::string(taskKind).c_str()));
  TFile f(Form("results/physlite/%d_throughput_%s.root", task, std::string(taskKind).c_str()), "RECREATE");
  canvas.Write();
}

void plot_throughput_physlite() {
  for (int i = 1; i <= 8; ++i) {
    plot(i, "compiled");
    // plot(i, "jitted");
  }
