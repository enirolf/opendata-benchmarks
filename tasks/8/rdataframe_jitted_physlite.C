...
unsigned int additional_lepton_idx(Vec<float> pt, Vec<float> eta, Vec<float> phi, Vec<float> mass, Vec<float> charge, Vec<int> flavour)
{
...
void rdataframe_jitted_ttree() {
    ...
    TCanvas cTTree;
    h->Draw();
    cTTree.SaveAs("8_rdf_physlite_ttree_jitted.png");
}
...
void rdataframe_jitted_rntuple() {
    ...
    TCanvas cRNTuple;
    h->Draw();
    cRNTuple.SaveAs("8_rdf_physlite_rntuple_jitted.png");
}
